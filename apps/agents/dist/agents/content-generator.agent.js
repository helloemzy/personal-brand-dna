...
exports.ContentGeneratorAgent = void 0;
...
class ContentGeneratorAgent extends base_agent_1.BaseAgent {
    openai;
    voiceProfiles = new Map();
    workshopDataCache = new Map();
    constructor() {
        super({
            type: shared_1.AgentType.CONTENT_GENERATOR,
    ...
    async generateVoiceMatchedContent(params) {
    ...
    buildSystemPrompt(voiceProfile, workshopData) {
...
Focus on ${values[0]} as the underlying theme.`;
    ...
    async humanizeContent(content, voiceProfile) {
        ...
        const hasSignaturePhrase = linguisticPatterns.signaturePhrases.some(phrase => humanized.toLowerCase().includes(phrase.toLowerCase()));
    ...
    async createContentVariations(content, voiceProfile, workshopData) {
        const variations = [];
...
5. Maintains authentic voice`;
    ...
    async calculateContentScores(content, voiceProfile, workshopData) {
        // Voice match score
    ...
    async calculateVoiceMatchScore(content, voiceProfile) {
        let score = 0.5; // Base score
        // Check for linguistic patterns
        const { linguisticPatterns } = voiceProfile;
        // Sentence starters (20%)
        const hasStarters = linguisticPatterns.sentenceStarters.some(starter => content.toLowerCase().includes(starter.toLowerCase()));
        if (hasStarters)
            score += 0.2;
        // Transitions (15%)
        const hasTransitions = linguisticPatterns.transitions.some(transition => content.toLowerCase().includes(transition.toLowerCase()));
        if (hasTransitions)
            score += 0.15;
        // Signature phrases (15%)
        const hasSignatures = linguisticPatterns.signaturePhrases.some(phrase => content.toLowerCase().includes(phrase.toLowerCase()));
        if (hasSignatures)
            score += 0.15;
        // Rhythm analysis (25%)
    ...
    async calculateRiskScore(content) {
        ...
        const hasControversial = controversialTerms.some(term => content.toLowerCase().includes(term));
        if (hasControversial)
            risk += 0.3;
        // Check for absolute statements
        const absoluteTerms = ['always', 'never', 'everyone', 'no one', 'all', 'none'];
        const absoluteCount = absoluteTerms.filter(term => content.toLowerCase().includes(term)).length;
        risk += Math.min(absoluteCount * 0.1, 0.3);
        // Check for sensitive topics
        const sensitiveTopics = ['salary', 'fired', 'lawsuit', 'confidential'];
        const hasSensitive = sensitiveTopics.some(topic => content.toLowerCase().includes(topic));
        if (hasSensitive)
            risk += 0.2;
    ...
    async getVoiceProfile(userId) {
        ...
        try {
            ...
            const workshopData = await workshop_data_service_1.workshopDataService.getWorkshopData(userId);
            if (!workshopData) {
                this.logger.warn({ userId }, 'No workshop data found for user');
                return null;
            }
            // Generate voice profile from workshop data
            const voiceProfile = await voice_profile_generator_service_1.voiceProfileGenerator.generateVoiceProfile(workshopData);
            // Cache it
            this.voiceProfiles.set(userId, voiceProfile);
            // Save to database for future use
            await workshop_data_service_1.workshopDataService.saveVoiceProfile(userId, voiceProfile);
            return voiceProfile;
        }
        catch (error) {
    ...
    selectRandomTopic(workshopData) {
    ...
    selectContentAngle(workshopData) {
        const angles = [
            'personal experience',
            'industry insight',
            'contrarian view',
    ...
    identifyContentPillar(topic, workshopData) {
    ...
    generateHashtags(content, workshopData) {
        const hashtags = [];
    ...
    describeTone(tone) {
        ...
        if (tone.formality > 0.7)
            ...
            descriptions.push('conversational');
        if (tone.analytical > 0.7)
            descriptions.push('analytical');
        if (tone.empathetic > 0.7)
            descriptions.push('empathetic');
        if (tone.assertive > 0.7)
            descriptions.push('assertive');
        return descriptions.join(', ');
    }
    getArchetypeMethod(archetype) {
        const methods = {
    ...
    insertFillerWord(content, filler) {
        const sentences = content.split('. ');
        if (sentences.length > 2) {
            // Insert in middle sentence
            const midIndex = Math.floor(sentences.length / 2);
            sentences[midIndex] = sentences[midIndex].replace(/^(\w+\s+\w+\s+)/, `$1${filler}, `);
        }
        return sentences.join('. ');
    }
    addSelfDeprecatingTouch(content) {
        const touches = [
    ...
    insertSignaturePhrase(content, phrase) {
        const sentences = content.split('. ');
    ...
    applyRhythmPattern(content, rhythmPatterns) {
    ...
    analyzeRhythm(content, targetRhythm) {
        ...
        if (targetRhythm.sentenceVariation === 'short-long-short') {
            ...
            for (let i = 1; i < lengths.length - 1; i++) {
                if (i % 2 === 1 && lengths[i] <= lengths[i - 1])
                    alternating = false;
                if (i % 2 === 0 && lengths[i] >= lengths[i - 1])
                    alternating = false;
            }
            if (alternating)
                score += 0.5;
    ...
    async analyzeToneMatch(content, targetTone) {
        ...
        if (targetTone.formality > 0.7 && hasFormal)
            score += 0.25;
        if (targetTone.formality < 0.3 && hasCasual)
            score += 0.25;
        // Analytical check
        ...
        if (targetTone.analytical > 0.7 && hasAnalytical)
            score += 0.25;
        return Math.min(score, 1);
    }
    checkValueAlignment(content, values) {
    ...
    calculateTimeliness(publishedDate) {
    ...
    async adjustContentStyle(content, style, voiceProfile) {
    ...
    async createStorytellingVariation(content, voiceProfile, workshopData) {
...
Keep it under 200 words and maintain the user's voice patterns.`;
    ...
    async updateVoiceProfile(userId, voiceProfile) {
        this.voiceProfiles.set(userId, voiceProfile);
    ...
    buildRevisionPrompt(originalContent, issues, suggestions, scores) {
...
exports.ContentGeneratorAgent = ContentGeneratorAgent;
//# sourceMappingURL=content-generator.agent.js.map
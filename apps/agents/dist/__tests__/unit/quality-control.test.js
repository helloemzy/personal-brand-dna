...
const content_safety_service_1 = require("../../services/content-safety.service");
const plagiarism_detector_service_1 = require("../../services/plagiarism-detector.service");
...
describe('QualityControlAgent Unit Tests', () => {
    ...
    beforeEach(async () => {
        ...
        content_safety_service_1.ContentSafetyService.mockImplementation(() => mockSafetyService);
        plagiarism_detector_service_1.PlagiarismDetector.mockImplementation(() => mockPlagiarismDetector);
        agent = new quality_control_agent_1.QualityControlAgent(messageBus);
        await agent.start();
    });
    afterEach(async () => {
        await agent.stop();
    ...
    describe('Quality Assessment', () => {
        it('should perform comprehensive quality check', async () => {
            ...
            const result = await agent.performQualityCheck(content);
        ...
        it('should fail content with safety issues', async () => {
            ...
            const content = {
                ...
                metadata: { userId: workshop_data_fixture_1.mockWorkshopData.userId }
            };
            const result = await agent.performQualityCheck(content);
    ...
    describe('Grammar and Readability', () => {
        it('should score grammar correctly', async () => {
            ...
            const goodResult = await agent.assessGrammar(wellWritten.text);
            const poorResult = await agent.assessGrammar(poorlyWritten.text);
        ...
        it('should assess readability', async () => {
            ...
            const readableScore = await agent.assessReadability(readable.text);
            const complexScore = await agent.assessReadability(complex.text);
    ...
    describe('Brand Alignment', () => {
        it('should check alignment with user brand', async () => {
            ...
            const alignedResult = await agent.assessBrandAlignment(aligned, workshop_data_fixture_1.mockWorkshopData);
            const misalignedResult = await agent.assessBrandAlignment(misaligned, workshop_data_fixture_1.mockWorkshopData);
        ...
        it('should verify content pillar alignment', async () => {
            ...
            const alignment = await agent.checkPillarAlignment(content, workshop_data_fixture_1.mockWorkshopData);
    ...
    describe('Engagement Prediction', () => {
        it('should predict content engagement', async () => {
            ...
            const engagingScore = await agent.predictEngagement(engaging.text);
            const boringScore = await agent.predictEngagement(boring.text);
    ...
    describe('Message Handling', () => {
        ...
        it('should handle batch quality checks', async () => {
            const batchRequest = {
                ...
                data: {
                    contents: [
                        { text: 'Content 1', metadata: { userId: workshop_data_fixture_1.mockWorkshopData.userId } },
                        { text: 'Content 2', metadata: { userId: workshop_data_fixture_1.mockWorkshopData.userId } },
                        { text: 'Content 3', metadata: { userId: workshop_data_fixture_1.mockWorkshopData.userId } }
                    ]
                }
            };
...
//# sourceMappingURL=quality-control.test.js.map
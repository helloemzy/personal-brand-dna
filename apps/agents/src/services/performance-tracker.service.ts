...
export class PerformanceTrackerService {
  ...
  private async fetchLinkedInMetrics(postId: string, userId: string): Promise<PerformanceMetrics> {
  ...
  private async fetchTwitterMetrics(postId: string, userId: string): Promise<PerformanceMetrics> {
  ...
  private async getBenchmarks(
    userId: string,
    platform: string
  ): Promise<{ accountAverage: PerformanceMetrics; industryAverage: PerformanceMetrics }> {
  ...
  private async getAccountAverage(userId: string, platform: string): Promise<PerformanceMetrics> {
  ...
  private async getIndustryAverage(platform: string): Promise<PerformanceMetrics> {
  ...
  private async storePerformance(data: any): Promise<void> {
    await this.supabase
      .from('content_performance')
      .insert({
  ...
  private async calculateTrends(performances: any[]): Promise<any> {
    ...
    const timeSlots = new Map<number, { engagement: number; count: number }>();
    const days = new Map<number, { engagement: number; count: number }>();
  ...
  private generateRecommendations(trends: any, performances: any[]): string[] {
    ...
    if (trends.bestTimes.length > 0) {
      ...
      recommendations.push(
        `Post at ${bestTime.hour}:00 for best engagement (${(bestTime.avgEngagement * 100).toFixed(1)}% average)`
      );
    }
    ...
    if (trends.bestDays.length > 0) {
      ...
      recommendations.push(
        `${days[bestDay.dayOfWeek]} shows highest engagement`
      );
    }
    ...
    if (trends.engagement.direction === 'down') {
      recommendations.push(
        'Engagement is declining - try varying content types and posting times'
      );
    } else if (trends.engagement.direction === 'up') {
      recommendations.push(
        `Great job! Engagement is up ${trends.engagement.percentage.toFixed(1)}%`
      );
    }
    ...
    if (avgScore < 50) {
      recommendations.push(
        'Consider adding more engaging elements like questions or polls'
      );
    }
  ...
  async getRealtimeMetrics(userId: string, platform: string): Promise<PerformanceMetrics> {
    const key = `realtime:${userId}:${platform}`;

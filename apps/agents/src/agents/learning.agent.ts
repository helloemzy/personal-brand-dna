...
import { Task, AgentType, AgentMessage, MessageType, Priority } from '@brandpillar/shared';
...
interface OptimizationUpdate {
  ...
  updateType: 'PARAMETER' | 'MODEL' | 'STRATEGY';
  updates: any;
  reason: string;
...
interface SystemInsights {
  ...
  recommendations: OptimizationUpdate[];
}
...
export class LearningAgent extends BaseAgent {
  ...
  private metricsCache: Map<string, PerformanceData[]> = new Map();
  ...
  constructor() {
    ...
    this.performanceTracker = new PerformanceTrackerService(this.logger);
  }
  ...
  private async analyzePerformance(request: LearningRequest): Promise<any> {
  ...
  private async optimizeAgent(request: { agentType: AgentType }): Promise<OptimizationUpdate[]> {
  ...
  private async generateInsights(request: LearningRequest): Promise<SystemInsights> {
  ...
  private async updateModels(request: { modelType: string; updates: any }): Promise<any> {
  ...
  private async optimizeSystem(request: LearningRequest): Promise<any> {
  ...
  private async analyzeExperiment(request: { experimentId: string }): Promise<any> {
    ...
    analysis.significance = this.calculateSignificance(
      analysis.control,
      analysis.treatment
    );
  ...
  private parseDuration(timeRange: string): number {
  ...
  private async identifyPatterns(data: PerformanceData[]): Promise<any> {
  ...
  private analyzeTemporalPatterns(data: PerformanceData[]): any {
  ...
  private analyzeContentPatterns(data: PerformanceData[]): any {
  ...
  private analyzeBehaviorPatterns(data: PerformanceData[]): any {
  ...
  private calculatePerformanceMetrics(data: PerformanceData[]): any {
    const metrics = {
    ...
    metrics.averageEngagement = data.reduce((sum, item) => 
      sum + (item.metrics.engagementRate || 0), 0) / data.length;
    ...
    const firstHalfAvg = firstHalf.reduce((sum, item) => 
      sum + (item.metrics.engagementRate || 0), 0) / firstHalf.length;
    
    const secondHalfAvg = secondHalf.reduce((sum, item) => 
      sum + (item.metrics.engagementRate || 0), 0) / secondHalf.length;
  ...
  private detectAnomalies(data: PerformanceData[]): any[] {
    const anomalies = [];
    ...
    const stdDev = Math.sqrt(
      engagementRates.reduce((sum, rate) => sum + Math.pow(rate - mean, 2), 0) / engagementRates.length
    );
  ...
  private async generateRecommendations(patterns: any, metrics: any, anomalies: any[]): Promise<string[]> {
    ...
    if (patterns.temporal.bestHours.length > 0) {
      ...
      recommendations.push(
        `Schedule posts at ${bestHour.key}:00 for optimal engagement (${bestHour.avg.toFixed(2)} average)`
      );
    }
    ...
    if (metrics.engagementTrend < -10) {
      recommendations.push(
        'Engagement is declining. Consider varying content types and increasing posting frequency.'
      );
    } else if (metrics.engagementTrend > 10) {
      recommendations.push(
        `Great progress! Engagement is up ${metrics.engagementTrend.toFixed(1)}%. Keep using similar strategies.`
      );
    }
    ...
    if (positiveAnomalies.length > 0) {
      recommendations.push(
        `Analyze your top ${positiveAnomalies.length} posts to identify success patterns.`
      );
    }
  ...
  private async sendOptimizationUpdate(update: OptimizationUpdate): Promise<void> {
  ...
  private getTopEntries(map: Map<number, number[]>, count: number): any[] {
  ...
  private getBottomEntries(map: Map<number, number[]>, count: number): any[] {
    return Array.from(map.entries())
      .map(([key, values]) => ({
        key,
        avg: values.reduce((sum, v) => sum + v, 0) / values.length,
  ...
  private async getAgentPerformance(agentType: AgentType): Promise<any> {
    const { data } = await this.supabase
      .from('agent_tasks')
      .select('*')
  ...
  private async identifyOptimizations(agentType: AgentType, performance: any): Promise<any[]> {
    const opportunities = [];
  ...
  private async generateOptimizationUpdate(
    agentType: AgentType,
    opportunity: any,
    performance: any
  ): Promise<OptimizationUpdate | null> {
  ...
  private async applyOptimization(update: OptimizationUpdate): Promise<void> {
  ...
  private async analyzeAllAgents(): Promise<any> {
    const analysis = {};
    
    for (const agentType of Object.values(AgentType)) {
      if (agentType === AgentType.LEARNING) continue; // Skip self
  ...
  private calculateTrends(tasks: any[]): any {
    if (tasks.length < 2) {
  ...
  private comparePeriods(firstPeriod: any[], secondPeriod: any[], metric: string): any {
  ...
  private identifyIssues(performance: any): string[] {
    const issues = [];
  ...
  private identifyBottlenecks(systemMetrics: any, agentPerformance: any): string[] {
    const bottlenecks = [];
  ...
  private async findOpportunities(systemMetrics: any, agentPerformance: any): Promise<string[]> {
    const opportunities = [];
    ...
    const avgEngagement = systemMetrics.content.reduce((sum, c) => 
      sum + (c.metrics?.engagementRate || 0), 0) / systemMetrics.content.length;
  ...
  private async generateSystemRecommendations(
    ...
    bottlenecks: string[],
    opportunities: string[]
  ): Promise<OptimizationUpdate[]> {
    const recommendations: OptimizationUpdate[] = [];
    
    // Address bottlenecks
    for (const [agentType, data] of Object.entries(agentPerformance)) {
      if (data.performance.averageDuration > 60000) {
        recommendations.push({
          targetAgent: agentType as AgentType,
          updateType: 'PARAMETER',
  ...
  private calculateSystemHealth(metrics: any): number {
    ...
    if (metrics.resources.memory.percentage > 80) health -= 10;
    if (metrics.resources.cpu.user > 80) health -= 10;
    
    // Deduct for agent issues
    metrics.agents.forEach(agent => {
      if (agent.performance.errorRate > 0.05) health -= 5;
      if (agent.performance.averageDuration > 60000) health -= 5;
    });
  ...
  private async validateModelUpdates(modelType: string, updates: any): Promise<any> {
    const errors = [];
  ...
  private async applyModelUpdates(modelType: string, updates: any): Promise<any> {
  ...
  private async testUpdatedModel(modelType: string): Promise<any> {
  ...
  private async rollbackModelUpdates(modelType: string): Promise<void> {
  ...
  private async analyzeSystemState(): Promise<any> {
    ...
    for (const agentType of Object.values(AgentType)) {
      if (agentType === AgentType.LEARNING) continue;
  ...
  private async getAgentHealth(agentType: AgentType): Promise<any> {
  ...
  private identifyOptimizationTargets(systemState: any): any[] {
    const targets = [];
  ...
  private async runOptimizationExperiment(target: any): Promise<any> {
    const experiment = {
      id: uuidv4(),
      target,
      startTime: new Date(),
  ...
  private createOptimizationFromExperiment(target: any, experiment: any): OptimizationUpdate {
    return {
      targetAgent: target.target as AgentType,
      updateType: 'PARAMETER',
      updates: {
  ...
  private async fetchExperimentData(experimentId: string): Promise<any> {
  ...
  private analyzeVariant(variant: any): any {
